...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((name, phone) in mapB) {
        if (mapA.contains(name) && rez[name] != phone) rez[name] = "${mapA[name]}, $phone"
        else rez[name] = phone
    }
    return rez
}
...
fun containsIn(a: Map<String, String>, b: Map<String, String>): Boolean {
    ...
    for ((key, _) in a) {
        if (key in b && a[key] == b[key]) return true
    }
    return false
}
 ...
 */
fun srValue(list: List<Double>): Double = if (list.isEmpty()) 0.0 else list.average()

fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    val first = mutableMapOf<String, Double>()
    val second = mutableMapOf<String, List<Double>>()
    for ((stock, price) in stockPrices) second[stock] = (second[stock] ?: emptyList()) + price
    for ((stock, price) in second) first[stock] = srValue(price)
    return first
}
/*  val second = mutableMapOf<String, Double>()
    val second = mutableMapOf<String, List<Double>>()
    for ((stock, price) in stockPrices) {
        if (second[stock] != null) {
            second[stock]!! + price
        } else {
            second[stock] = emptyList()
        }
    }
    for ((stock, price) in pres) {
        second[stock] = srValue(price)
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    return rez.filter { (ch, int) -> int > 1 }
}
